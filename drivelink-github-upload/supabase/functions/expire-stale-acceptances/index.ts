...
// parties agreed to.
//
// AUTH: this endpoint cancels sellers' accepted offers, so it requires a
// bearer token. It previously accepted any request that reached the URL — the
// listing-state guard below limited what an attacker could destroy, but that
// is a blast-radius argument, not access control.
//
// The token is CRON_SHARED_SECRET, generated by us and stored in both Vault
// (for the pg_cron job to read) and the Edge Function secrets. It is NOT the
// service-role key. That was the first attempt, and it failed: Supabase
// injects SUPABASE_SERVICE_ROLE_KEY into the function itself, and on a project
// migrated to JWT signing keys the injected value is not necessarily the same
// string as the legacy service_role key shown on the API Keys page — so the
// function spent an hour returning 401 to its own scheduler while both sides
// held a valid key. One secret, one source of truth, no dependency on which
// key Supabase happens to inject.
//
...
// Still needed for createClient — this function writes past RLS by design.
const SERVICE_ROLE = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;
// Authenticates the caller. See the AUTH note in the header for why this is
...
Deno.serve(async (req) => {
  // 0. Authorization. The `!expected` arm matters as much as the comparison:
  //    if CRON_SHARED_SECRET were ever unset, a naive check would accept a
  //    bare "Bearer " and hand this endpoint to anyone. Refuse instead.
