...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (ref) {
      ...
      // (promoter_id === buyer_id) should never earn a commission. Flag for
      // admin review instead of transferring money automatically — same
      // policy as the manual (non-Stripe) confirmReceipt() path.
      const isSelfReferral = ref.promoter_id === listing.buyer_id;
      if (isSelfReferral) {
        await supabase
          .from("referrals")
